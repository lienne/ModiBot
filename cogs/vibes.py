...
class Vibes(commands.Cog):
    ...
    async def vibe(self, ctx, target : discord.Member = None):
        ...
        vibes = [':crown: Top', ':peach: Bottom', ':full_moon_with_face: Strange', ':rose: Charm', ':boxing_glove: Chutzpah',
        ...
        h = hashlib.sha256()
        h.update(target.id.to_bytes(8, 'big'))

        values = [x % 100 for x in list(h.digest())[7:15]]
